...
function normalizeListing(l, idx) {
  ...
  const condLabel = l.condition === 'new' ? 'New' : l.condition === 'used' ? 'Used' : '';
  const specs = [
    trim || null,
...
async function runSearch(page = 1, forceParams = {}) {
  ...
  async function fetchBothConditions(p) {
    // Global sorts use auto.dev v2, which sorts the complete matching result
    // set before pagination and can return new + used together.
    if (currentSort !== 'default' && currentSort !== 'distance-asc') {
      const d = await fetchInventory({ ...p, sort: currentSort }, page, PAGE_SIZE);
      const sortedRecords = d.data || d.listings || d.records || [];
      if (sortedRecords.length > 0) {
        return { records: sortedRecords, total: d.totalCount || 0 };
      }
      // auto.dev v2 occasionally lacks listings that exist in its legacy
      // inventory (notably new BMW ALPINA XB7s). Retry without server sorting;
      // the returned fallback records are sorted locally before rendering.
      const fallback = await fetchInventory(p, page, 100);
      return {
        records: fallback.data || fallback.listings || fallback.records || [],
  ...
  try {
    ...
    // 1) One query per selected regular trim. auto.dev does not support
    //    comma-separated trim values, so querying the unfiltered model would
    //    put unrelated vehicles on the page before local filtering.
    // 2) One query per M-alias trim (each M variant is its own model in auto.dev)
    const fetchPromises = [];

    if (regularTrimArr.length > 0) {
      regularTrimArr.forEach(selectedTrim => {
        fetchPromises.push(fetchBothConditions({ ...sharedParams, model, trim: selectedTrim }));
      });
    } else if (mAliasTrimArr.length === 0) {
...
function setSort(sort, btn) {
  ...
  // Re-query from page 1 whenever ordering changes. Server-supported sorts
  // are applied to the full matching inventory before pagination.
  runSearch(1);
}
      ...
      <div class="car-body">
        ...
        <div class="car-meta">
          <div>
            ${
              car.vin
                ? `<a class="car-name vehicle-detail-link" href="/vehicle/${encodeURIComponent(car.vin)}" onclick="event.preventDefault();event.stopPropagation();openDetail(${cid})">${escHtml(car.year+' '+car.name)}</a>`
